...
const createReview = async function (req, res) {
  try {
    ...
    if ( !rating)
      return res
        .status(400)
        .send({ status: false, message: 'Review rating is required' });
    ...
    const newReview = await reviewModel.create(reviewDetail1);
    const updatedReviews = await reviewModel
      .find({_id:newReview._id,bookId:bookId,isDeleted:false})
...
const deleteReviews = async function (req,res){
  try {
    ...
    await bookModel.findById( bookId ).updateOne({ $inc: { reviews: -1 } });
